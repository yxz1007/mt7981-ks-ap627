...
{
	const uint32_t image_len = getFileSize(filename);

	FILE* file = fopen(filename, "rb");
	if (file == NULL)
	{
		LOG_E("can not open %s", filename);
		return false;
	}
	uint32_t crc32 = 0xFFFFFFFFu;
	uint32_t i = 0;
	...
	for (i = 0; i < image_len; i += buff_size)
	{
		...
		else
		{
			...
			if (m != 0)
			{
				check_length = check_length + (4 - m);//must 4-byte alignment.
			}
		}

		result = fread(&buff, check_length, 1, file);
		if (result == 0)
		{
			LOG_I("Fill up 4-byte. ( %d )", (int)result);
			//return 0;
		}
