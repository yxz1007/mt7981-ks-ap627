...
if [ -n ${1} ]; then
	...
	if [ "$1" = "merlin-2g-5g" ]; then
		echo "CONFIG_MTK_SKU_AX5400_255=y" >> autobuild/${branch_name}/.config
		sed -i 's/mt7916-ax5400.dbdc.b1.dat/mt7916-ax5400-5G.dbdc.b1.dat/g' autobuild/${branch_name}/.config
		need_build=1
	fi
