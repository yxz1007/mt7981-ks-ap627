...
#define DEBUG
#ifdef DEBUG
#define MTK_FPGA_DBG(fmt, ...)	dev_info(u2phy->dev, fmt, ##__VA_ARGS__)
#else
...
#define PHY_RAEDB(addr)		usb_phy_readb(addr)
...
static unsigned int get_phy_version(struct phy *phy)
{
	...
	struct a60810_u2phy *u2phy = dev_get_drvdata(phy->dev.parent);
	unsigned int version = 0;
	void *i2c_port_base;

	i2c_port_base = instance->port_base;

	u3phy_write_reg8(g_ippc_port_addr,
			 A60810_I2C_ADDR, 0xff, PHY_VERSION_BANK);

	version = u3phy_read_reg32(g_ippc_port_addr,
				   A60810_I2C_ADDR, PHY_VERSION_ADDR);
	dev_info(u2phy->dev,
...
static int a60810_init(struct phy *phy)
{
	...
	if (get_phy_version(phy) != 0xa60810a) {
		dev_info(u2phy->dev, "get phy version failed\n");
		return -1;
	}
...
static int a60810_phy_init(struct phy *phy)
{
	...
	ret = clk_prepare_enable(u2phy->u3phya_ref);
	if (ret) {
		dev_err(u2phy->dev, "failed to enable u3phya_ref\n");
		return ret;
	}

	ret = clk_prepare_enable(instance->ref_clk);
	if (ret) {
		dev_err(u2phy->dev, "failed to enable ref_clk\n");
		return ret;
	}
...
static const struct phy_ops a60810_u2phy_ops = {
...
static const struct of_device_id a60810_u2phy_id_table[] = {
	{
...
MODULE_DEVICE_TABLE(of, a60810_u2phy_id_table);

static void phy_v1_banks_init(struct a60810_u2phy *u2phy,
...
static void phy_v2_banks_init(struct a60810_u2phy *u2phy,
	struct a60810_phy_instance *instance)
{
	struct u2phy_banks *u2_banks = &instance->u2_banks;
	struct u3phy_banks *u3_banks = &instance->u3_banks;
...
{
	struct a60810_u2phy *u2phy = dev_get_drvdata(dev);
	...
	for (index = 0; index < u2phy->nphys; index++)
		if (phy_np == u2phy->phys[index]->phy->dev.of_node) {
			instance = u2phy->phys[index];
			break;
		}
	...
	if (u2phy->pdata->version == MT_PHY_V1) {
		phy_v1_banks_init(u2phy, instance);
	} else if (u2phy->pdata->version == MT_PHY_V2) {
		phy_v2_banks_init(u2phy, instance);
	} else {
...
static int u2phy_fpga_a60810_probe(struct platform_device *pdev)
{
	...
	struct a60810_u2phy *u2phy;
	struct resource res;
	int port, retval;

	match = of_match_node(a60810_u2phy_id_table, pdev->dev.of_node);
	if (!match)
		return -EINVAL;

	u2phy = devm_kzalloc(dev, sizeof(*u2phy), GFP_KERNEL);
	if (!u2phy)
		return -ENOMEM;

	u2phy->pdata = match->data;
	u2phy->nphys = of_get_child_count(np);
	u2phy->phys = devm_kcalloc(dev, u2phy->nphys,
				       sizeof(*u2phy->phys), GFP_KERNEL);
	if (!u2phy->phys)
		return -ENOMEM;
	...
	if (u2phy->pdata->version == MT_PHY_V1) {
		...
		u2phy->sif_base = devm_ioremap_resource(dev, sif_res);
		if (IS_ERR(u2phy->sif_base)) {
	...
	u2phy->u3phya_ref = devm_clk_get(dev, "u3phya_ref");
	if (IS_ERR(u2phy->u3phya_ref)) {
		if (PTR_ERR(u2phy->u3phya_ref) == -EPROBE_DEFER)
			return -EPROBE_DEFER;

		u2phy->u3phya_ref = NULL;
	}
	...
	for_each_child_of_node(np, child_np) {
		...
		u2phy->phys[port] = instance;

		phy = devm_phy_create(dev, child_np, &a60810_u2phy_ops);
		if (IS_ERR(phy)) {
...
static struct platform_driver a60810_u2phy_driver = {
	.probe		= u2phy_fpga_a60810_probe,
	.driver		= {
		.name	= "a60810-u2phy",
		.of_match_table = a60810_u2phy_id_table,
	},
};

module_platform_driver(a60810_u2phy_driver);
